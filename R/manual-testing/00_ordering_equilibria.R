...
#'
#' This file contains manual testing for the equilibria matching process. The
#' original code itself was written by Ariel Griener, and has been trasnitioned 
#' here into the targets framework, by wrapping it all in a function, without 
#' regard for efficiency or piece-mealing
#'
...
order_match_equil <- function(data, output_path) {
...
#note: the nice thing about just doing stable nodes is that they 
...
dim(onlystabnodes) #26699 x 15...we know that there is one stable node per 
#param_combo at least so 6095 have more than 1

#i think that there is a much easier way to do this than the code that is being 
#used below but im just going to do it this way for now and see what happens


#determine starting values
#data$ID[data$paramcombo == 1 & data$stability == "stable_node"] <- 
#data$Equilibrium[data$paramcombo == 1 & data$stability == "stable_node"]

#things get messed up below if this doesn't start at 1
data$ID[data$paramcombo == 1 & 
          data$stability == "stable_node"] <- 1 
numequi_old <- length(data$Equilibrium[data$paramcombo == 1 
                                       & data$stability == "stable_node"]) 
equival_storage <- data.frame(ID_num=seq(1,paramcomb, by=1), 
...
#check whether at bifurcation point or not (change in number of equi, 
#OTHER THINGS?)
#check which of the next set of equilibria correspond to which of the first 
#set, assign them IDs in line w said identification

#ran into some issue at i = 18 (fixed by assigning the first ID to 1), issue 
#with 3123 bc noo stable node so need to skip
for(i in 2:20604){ #2:paramcomb
  ...
  numequi <- length(data$Equilibrium[data$paramcombo == i 
                                     & data$stability == "stable_node"]) 
  equivals <- data.frame(EquiNum=seq(1,numequi, by=1), 
                         C = NA, M = NA, matched = 0) #changed from matched <- 0
  ...
  if(numequi == numequi_old){ #match the IDs
    ...
    #issue: ^ after hit a 'no matches' an NA is introduced and doesn't go away 
    #until there's a bifurcation
        ...
        if(equivals$matched[j] < 1){ #not currently matched
          ...
          for(k in 1:numID){ 
            ...
            if(((Cone + (prop_cutoff*Cone + cutoff_add) > ConeID) & 
              ...
              #keep thinking about whether this is a problem to re-define 
              #thesee within the loop - made it so can't redo ID == k
  ...
  if(numequi != numequi_old){ 
    #bifurcation, try to continue assigning old IDs 
    #...assign new ones otherwise
    ...
    if(sum(equivals$matched) < numequi){
      ...
      for(j in 1:numequi){
        ...
        if(equivals$matched[j] < 1){ 
          ...
          for(k in 1:numID){ 
            #shouldn't get repeat values from this bc same criteria as above 
            #for the already set IDs
      ...
      #ok so it goes through all the unmatched ones and tries to match 
      #them with a previous ID, if some are still unmatched...
    }	
