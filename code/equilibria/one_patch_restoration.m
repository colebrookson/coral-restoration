...
ep_zeta_vec = repmat([0.005 0.01 0.05 0.1 0.5], [20 99]);
ep_zeta_vec = ep_zeta_vec(:)
...
data(1:end,2) = recr_vec % gamma
data(1:end,3) = recr_vec % d
data(1:end,4) = equinum % a
data(1:end,5) = gvecvec % epsilon
data(1:end,6) = recr_vec % zeta
...
init_guess = [0 1; 0 1];%init guess says that the state variable 
%are bounded between 0 and 1 so it bounds the search space
S = vpasolve(eqns, [x,y], init_guess) %some sol'ns with numerical instabilities but those are all ones outside the specified range
